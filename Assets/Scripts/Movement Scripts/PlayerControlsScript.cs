...
public class PlayerControlsScript : MonoBehaviour
{
    ...
    {
        GameManager.screenFlasher.FadeToColor(1, new Color(0,0,0,1));
    ...
    {
        edge = Mathf.Min(edge+1, maximumEdge);
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
         ...
         {
            originalPosition = (this.transform.position - originalPosition) * 0.25f + originalPosition;
            chasmCount++;
         }
    }
    ...
    IEnumerator DoDash()
    {
        ...
        // if dash finished without getting interrupted by a hit
        if (stateManager.CurrentState == State.Dashing)
        {
            if (inAir)
            {
                if (stateManager.TrySetState(State.Falling))
                {
                    yield return StartCoroutine("DoFall");
                }
            }
        ...
        Physics2D.IgnoreLayerCollision(Layer.PlayerFeet, Layer.EnemyFeet, false); // reactivate collisions between player and enemy feet
